...
String apiKey = "AJNSVIA6XSCN3V5I"; // Use your correct Write API Key
...
void setup() {
  ...
  // Perform an initial "dummy read" to wake up the NPK sensor
  Serial.println("Performing initial 'dummy' read of NPK sensor...");
  readNPK();
  delay(1000); // Wait a moment after the dummy read.
}

void loop() {
  readNPK();
  
  // Increased delay to allow power supply to stabilize before reading the second RS485 sensor.
  delay(3000); 
...
byte readSpecificNutrient(const byte* request, byte req_len, byte* response) {
  ...
  // Increased timeout to 1.5 seconds for more reliability
