/*
 * Combined ESP32 Soil & Environment Monitoring System (Final Cleaned Version)
 *
 * This sketch reads data from multiple sensors, displays it on a local OLED screen,
 * and uploads it to a ThingSpeak channel. This is the final, optimized version.
 *
...
void setup() {
  ...
  display.setCursor(0, 0); display.println("System Monitor v4.0");
...
void loop() {
  ...
  printAllToSerial();
  updateOLED();
  oled_screen_toggle = !oled_screen_toggle; // Switch OLED screen for the next loop

  // --- Send to Cloud ---
  sendAllToCloud();

  // ThingSpeak has a rate limit of ~15 seconds. 20s is a safe interval.
  delay(20000); 
}
...
float readCapacitiveMoisture() {
  ...
  // Constrain the value to ensure it stays within the 0-100% range
...
void readSoilSensor() {
    byte response[9]; // Buffer sized for the largest response
    ...
    delay(500); // Small delay between sensor commands
...
void sendAllToCloud() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Brief delay to allow the request to be sent before checking the response
    delay(250); 

    // Check for a response from the server to confirm the update
    ...
    // A response was received, so the update was likely successful.
    Serial.println("ThingSpeak update sent.");
    client.stop(); // Cleanly close the connection
  } else {
